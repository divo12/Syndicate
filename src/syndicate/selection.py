...
from syndicate.comparison_contracts import Arm, TrialOutcome
...
def assess_comparison(
    ...
    if not _complete(policy, measurements):
        return _assessment(
            ComparisonDecision.INCONCLUSIVE, incumbent, candidate, "evidence-incomplete"
        )
    if (
        ...
        return _assessment(
            ComparisonDecision.INCONCLUSIVE, incumbent, candidate, "cost-unknown"
        )
    if _regressed(policy, measurements):
        return _assessment(
            ComparisonDecision.RETAIN, incumbent, candidate, "task-regression-floor"
        )
    if _promotable(policy, incumbent, candidate):
        return _assessment(
            ComparisonDecision.PROMOTE, incumbent, candidate, "supported-improvement"
        )
    return _assessment(
        ComparisonDecision.RETAIN, incumbent, candidate, "selection-rule-not-met"
    )
