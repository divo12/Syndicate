"""Atomic accepted-harness pointer and immutable promotion/rollback lineage."""
...
class HarnessLineage:
    ...
    def __init__(
        ...
        with self._connect() as connection:
            ...
            connection.execute(
                ...
                action TEXT NOT NULL, previous_harness TEXT NOT NULL,
    ...
    def promote(
        ...
        with self._connect() as connection:
            ...
            self._record(connection, operation_id, "promote", previous, candidate)
            return PromotionReceipt(
                operation_id=operation_id,
    ...
    def rollback(self, operation_id: UUID, harness_hash: str) -> PromotionReceipt:
        with self._connect() as connection:
            ...
            self._record(connection, operation_id, "rollback", previous, target)
            return PromotionReceipt(
                operation_id=operation_id,
    ...
    def history(self) -> tuple[PromotionReceipt, ...]:
        with self._connect() as connection:
            rows = connection.execute(
                """SELECT operation_id, action, previous_harness, previous_memory,
    ...
    def _receipt(self, row: tuple[str, str, str, str, str, str]) -> PromotionReceipt:
        status = (
            ...
            if row[1] == "promote"
            else PromotionStatus.ROLLED_BACK
        )
        return PromotionReceipt(
